...
test('multi element, different props', t => {
  ...
  return reshape({ plugins: [components({ 'my-component': MyComponent })] })
    ...
    .then(res => {
      t.is(
        res.output(),
        '<p>the value of foo is "bar"</p><p>wow</p><p>the value of foo is "wow"</p>'
      )
    })
})
...
test('renders children', t => {
  ...
  const html =
    "<my-component foo='bar'><p>wow</p><div><c2></c2></div></my-component>"
  ...
  return reshape({ plugins: [components({ 'my-component': MyComponent, c2 })] })
    ...
    .then(res => {
      t.is(
        res.output(),
        '<div class="parent">hello<p>wow</p><div><div class="wow">hello from c2</div></div></div>'
      )
    })
})
